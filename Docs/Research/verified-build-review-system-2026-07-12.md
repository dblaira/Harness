...
Security amendment (2026-07-13): XCUITest remains part of clean hosted CI, but proposal-owned UI-test code no longer runs in Adam's signed-in macOS session. Hosted CI executes the exact committed UI-test identifier and a fresh protected-base job revalidates the raw result bundle and exported screenshot. The signed handoff uses an immutable installed Accessibility driver that accepts only committed `wait_for`, `press`, `set_value`, and `assert_not_present` actions, binds every target to the exact sandboxed candidate PID, and records only its verified window. Proposal manifests, project generation, dependency resolution, compilation, tests, and app processes run with throwaway home directories inside outer macOS sandboxes that deny Adam's operator home, clipboard, Apple Events, installed controls, live ontology writes, verifier-owned evidence paths, and network except during the separately confined dependency-fetch step. Every proposal build, test, and app command also runs in a launchd resource coalition, allowing the immutable wrapper to enumerate and remove descendants even after they clear their environment, create another process group, or reparent; a regression probe proves that the outer sandbox remains enforced inside that coalition. A build-phase adversarial probe must prove that repository, control, home, accepted-graph, network, and launchd escape attempts are denied. Proposal test and app sandboxes additionally deny process creation entirely. The Stop gate invalidates proof whenever the current worktree changes and permits a clean completion claim only after the final merged-tree proof exists.
...
Ten independent local GPT-5.6 Sol reviews were used during the initial bootstrap. The first nine successively exposed false-proof, fail-open, trust-boundary, authority-route, stale-contract, evidence-binding, and Stop-hook defects; the tenth found four remaining paths: a proposal-editable live Fuseki/Ollama oracle, feature tests not wrapped by immutable PID/window evidence, unreadable Git in secondary Harness worktrees, and Git-quoted Swift paths bypassing Periphery. A subsequent exact-commit review found four more P1 defects: compilation outside the proposal sandbox, no hosted execution of the contracted XCUITest, stale Stop proof after new worktree changes, and detached descendants escaping process-group cleanup. Actionable findings were converted into code and adversarial regressions. The production chat change was removed from this infrastructure PR; Fuseki is instead proved by a separate accepted-only live preflight. The sixth review's request for resistance to intentional misuse of Adam's own administrator credential was resolved separately through Adam's explicit no-Touch-ID threat-model decision. This is the intended operating model: Sol supplies adversarial findings, the owner defines the authority boundary, and executable evidence decides whether each correction is real.
...
- [Apple XNU coalition observability](https://github.com/apple-oss-distributions/xnu/blob/main/doc/observability/coalitions.md) — coalition membership survives fork, exec, and process spawning and represents a launchd job.
