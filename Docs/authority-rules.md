# Authority Rules
Agents may cite `/accepted` as Adam's truth.
`/accepted` means `https://understood.app/graph/accepted`.
Only Adam-approved Harness TTLs belong there.
`/candidates` means `https://understood.app/graph/candidates`.
`/candidates` is context only.
When using `/candidates`, label it unreviewed.
